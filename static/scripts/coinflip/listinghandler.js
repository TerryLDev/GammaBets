...
const gameArea = document.getElementById('cf-game-area');
...
socket.on('coinFlipLoader', games => {
    ...
    games.forEach(game => {
        ...
        // this should remove the game from the website after it has been flipped
            ...
            gameArea.removeChild(document.getElementById(game.gameID))
...
function addNewCFGame(cf) {
    ...
    coinFlipGameListing.className = "cf-game-listing"
    coinFlipGameListing.id = String(cf.gameID);

    // image element for player profile image
    let playerImage = document.createElement("img");
    playerImage.className = "cf-game-info-picture";
    ...
    gameArea.appendChild(coinFlipGameListing);
...
async function buildView(game) {
    ...
    for (let i = 0; i < game.playerOneSkins.length; i++){
        ...
        newSkin.innerHTML = "<img src='" + game.playerOneSkinPictures[i] + "' alt='" + game.playerOneSkins[i] + "'/><p>" + game.playerOneSkins[i] + "</p><p>$" + skinPrice + "</p>"
